...
class Recentview {
	...
	public function __construct($registry) {
		...
		if (!isset($this->session->data['recentview']) || !is_array($this->session->data['recentview'])) {
			$this->session->data['recentview'] = array();
		}
	}

	public function getRecentViewedProducts() {
		if (!$this->data) { 
			foreach ($this->session->data['recentview'] as $key => $quantity) { 
				...
				// $product_query = $this->db->query("SELECT *, pd.name AS name, (SELECT st.name FROM " . DB_PREFIX . "product_to_store pts LEFT JOIN " . DB_PREFIX . "store st on pts.store_id = st.store_id where p.product_id = pts.product_id AND st.store_id != 0 ) AS vendor FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = '1'");
				...
				if ($product_query->num_rows) {
					...
					foreach ($this->session->data['recentview'] as $key_2 => $quantity_2) {
					...
					$this->data[$key] = array(
						...
						// 'vendor'          => ucfirst($product_query->row['vendor']),
						...
						// 'price_range'           => $product_query->row['price_range'],
	...
	public function getRecurringProducts() {
		...
		foreach ($this->getRecentViewedProducts() as $key => $value) {
			if ($value['recurring']) {
	...
	public function add($product_id, $qty = 1, $option = array(), $recurring_id = 0) {
		...
		unset($this->session->data['recentview'][$key]); 

		if ((int)$qty && ((int)$qty > 0)) {  
			if (!isset($this->session->data['recentview'][$key])) {
				$this->session->data['recentview'][$key] = (int)$qty;
			} else {
				$this->session->data['recentview'][$key] += (int)$qty;
			}
		}
	...
	public function update($key, $qty) {
		...
		if ((int)$qty && ((int)$qty > 0) && isset($this->session->data['recentview'][$key])) {
			$this->session->data['recentview'][$key] = (int)$qty;
		} else {
			$this->remove($key);
		}
	...
	public function getWeight() {
		...
		foreach ($this->getRecentViewedProducts() as $product) {
			if ($product['shipping']) {
	...
	public function countProducts() {
		...
		$products = $this->getRecentViewedProducts();
	...
	public function hasProducts() {
		return count($this->session->data['recentview']);
	}
